...
# 1. 생각의 분해: 3단계로 탐색한다.
# 2. 생각 만들기: 이전 단계의 생각을 구체적으로 구현하는 5가지 방안을 생성한다.
# 3. 상태 평가: 단계별로 다음의 항목으로 평가한 후 최고점을 얻은 노드만 남긴다.
#   1) 창의적이고 혁신적인 방법인가?
#   2) 단기간에 실현 가능한 방법인가?
...
report = request_gpt(summary, "gpt-3.5-turbo", temperature=0, type="text")
print(report)
